...
maxT=''
i=0
for paperID,title in all_titles.items():
    i+=1
    ...
    if(sim1>maxS):
        ...
        maxP=paperID
        maxT=title
    journal = all_journals[paperID]
    all_titles[paperID] = [title,journal,sim1,sim2]
    # if(i==2):
        # break
...
for k,v in all_titles.items():
    # print v
    new.write(str(k) + ',"' + v[0] + '","' + str(v[1]) +'",' + str(v[2])+',' + str(v[3])+ "\n")
    i+=1
    # if(i==2):
        # break
print i