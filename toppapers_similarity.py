...
sum2=0
i=0
with open("output/sample61/filtered_papers_titSim.csv",'r') as file:
	for line in file:
		i+=1
		if(i==1):
			continue
...
new=open("output/sample61/filtered_papers_titSim_or_avg.csv",'w')
i=0
for k,v in pdetails.items():
	# i+=1
	if( k in paperids_or):
...
sim2_pdetails={}
i=0
with open("output/sample61/papers_without_refernces_titSim.csv",'r') as file:
	for line in file:
		i+=1
		if(i==1):
			continue
...
new=open("output/sample61/paperids_without_ref_or_20.txt",'w')
i=0
for k in topset:
	i+=1
	new.write(k+'\n')
