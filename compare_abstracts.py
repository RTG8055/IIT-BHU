...
doc_set=[]
docid_paperid={}
file = open('output/sample61/paperids61_abstract.txt','r')
i=0
for line in file:
	i+=1
	line = line.strip('\n|\r').split('\t')
...
journal_set={}
with open("output/sample61/paperids61_journals_conf_title.txt",'r') as f:
	i=0
	for line in f:
		i+=1
		line = line.strip("\n")
...
sample23:
Semi-naive Bayesian techniques seek to improve the accuracy of naive Bayes (NB) by relaxing the attribute independence assumption. We present a new type  of semi-naive Bayesian operation, Subsumption Resolution (SR), which efficiently identifie  s occurrences of the specialization-generalization relationship and eliminates generalizations at class  fication time. We extend SR to Near-Subsumption Resolution (NSR) to delete near generalizations in addit  ion to generalizations. We develop two versions of SR: one that performs SR during training, called eage  r SR (ESR), and another that performs SR during testing, called lazy SR (LSR). We investigate the effect   of ESR, LSR, NSR and conventional attribute elimination (BSE) on NB and Averaged One-Dependence Estimat   ors (AODE), a powerful alternative to NB. BSE imposes very high training time overheads on NB and AODE    accompanied by varying decreases in classification time overheads. ESR, LSR and NSR impose high training    time and test time overheads on NB. However, LSR imposes no extra training time overheads and only mode    st test time overheads on AODE, while ESR and NSR impose modest training and test time overheads on AOD    E. Our extensive experimental comparison on sixty UCI data sets shows that applying BSE, LSR or NSR to     NB significantly improves both zero-one loss and RMSE, while applying BSE, ESR or NSR to AODE significa    ntly improves zero-one loss and RMSE and applying LSR to AODE significantly improves zero-one loss. The     Friedman test and Nemenyi test show that AODE with ESR or NSR have a significant zero-one loss and RMSE      advantage over Logistic Regression and a zero-one loss advantage over Weka's LibSVM implementation with      a grid parameter search on categorical data. AODE with LSR has a zero-one loss advantage over Logistic R     egression and comparable zero-one loss with LibSVM. Finally, we examine the circumstances under which the      elimination of near-generalizations proves beneficial.
...
new = open("output/sample61/scikit_nmf61.txt",'w')
# for d in nmf_order:
# 	pid = docid_paperid.get(d)
# 	print d,pid
# 	if(pid != None):
...
new = open("output/sample61/scikit_lda61.txt",'w')
# for d in lda_order:
# 	pid = docid_paperid.get(d)
# 	print d,pid
# 	if(pid != None):
...
new = open("output/sample61/LN61.txt",'w')
i=0
written_journal=set()
for d in doc_list:
	# i+=1
